...
% hence one needs to specify a meaningful property of time-series data (as many as yi defined): 
y1s = y1(:);
y2s = y2(:);

%% compile an model output matrix
Sim = [y1s y2s];   %as big as yi defined
[n k] = size(X) ;
n = n / (k+1);
[mm ll] = size(Sim) ;
...
var = {'Electricity{ }Price{}2020','Electricity{ }Price{}2030'} ; %user defined name of variables
...
for i=1:n
    for j=1:k
        ...
        %Original method (calculate as many EEi as yi, manually changed by
        %the user)
        dtheta = Xval(r1+1,ix) - Xval(r1,ix) ;
        EE1(i,ix) = (((Sim(r1+1,1) - Sim(r1,1))/dtheta * (sig_x(ix)/sig_y(1))))  ; % SEEi = EEi* sigx/sigy where EEi = (Ytheta - Ytheta+dtheta) / dtheta *
        EE2(i,ix) = ((Sim(r1+1,2) - Sim(r1,2))/dtheta * (sig_x(ix)/sig_y(2)));
        
        %Alternative method using mean values of input parameter to scale
        %results (calculate as many EEi as yi, manually changed by
        %the user)
%         dtheta= (Xval(r1+1,ix) - Xval(r1,ix))/pmor(ix);
%         EE1(i,ix) = (((Sim(r1+1,1) - Sim(r1,1))/dtheta )); % SEEi = EEi* sigx/sigy where EEi = (Ytheta - Ytheta+dtheta) / dtheta *
%         EE2(i,ix) = ((Sim(r1+1,2) - Sim(r1,2))/dtheta  );
